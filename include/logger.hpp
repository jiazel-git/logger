...
public:
    logger( const std::string& _Path, const std::string& _Filepre = "log",
            const std::size_t _Size =
                1024 ) :  //_Fstream( _Filepre, std::ios_base::out | std::ios_base::app ),
        _File_path( _Path ),
        _Cur_file_idx( 0 ),
        _Log_size( _Size ),
        _File_pre( _Filepre ),
            ...
            for ( ;; ) {
                ...
                std::for_each( _Tmp->cbegin(), _Tmp->cend(), [ this ]( const auto& _Item ) {
                    ...
                    if ( !std::filesystem::exists( _File_path + "/" + _File ) ) {
                        ...
                        int s = _Log_size - std::filesystem::file_size( _File_path + "/" + _File ) -
                                1 - _Head.size() - _Item._Buffer.size();

                        std::cout << "file_size="
                                  << std::filesystem::file_size( _File_path + "/" + _File ) << " "
                                  << s << std::endl;
                        bool _Is_need_create =
                            ( _Log_size - std::filesystem::file_size( _File_path + "/" + _File ) -
                              1 - _Head.size() - _Item._Buffer.size() ) <= 0
                                ? true
                                : false;
                        if ( _Is_need_create ) {
                            std::cout << "file_size=="
                    ...
                    _Fstream.open( _File_path + "/" + _File,
                    ...
                    if ( _Fstream.is_open() ) {
                        ...
                        _Fstream.write( "\n", 1 );
                        _Fstream.flush();
                        std::cout << "write " << _Item._Buffer.c_str() << " to " << _File
                                  << std::endl;
                    }
                } );
            }
        } ),
        _Buffers( std::make_unique< _Buf_Vec >() ) {
...
private:
    [[nodiscard]] std::string _Prepare_head( int _Level ) {
        ...
        std::stringstream ss;
        ss.fill( '0' );
        ss << "[" << std::setw( 4 ) << 1900 + time.tm_year << std::setw( 2 ) << 1 + time.tm_mon
           << std::setw( 2 ) << std::setw( 2 ) << time.tm_mday << "-" << time.tm_hour << ":"
           << std::setw( 2 ) << time.tm_min << ":" << std::setw( 2 ) << time.tm_sec << "]"
           << "[" << _Logstr[ _Level ] << "]";
        ss >> _Head;
    ...
    void _Create_file() {
        ...
        std::stringstream ss;
        ss.fill( '0' );
        ss << std::setw( 4 ) << 1900 + _Time.tm_year << std::setw( 2 ) << 1 + _Time.tm_mon
           << std::setw( 2 ) << std::setw( 2 ) << _Time.tm_mday;
...
private:
    inline static std::vector< std::string > _Logstr{ "info", "trace", "error", "warning",
                                                      "debug" };
};
}  // namespace jz
